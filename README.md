...
- Verze aplikace: `4.3.0` podle `battle-plan/package.json`.
...
Pro dalsi praci s Compound Engineering pluginy zacni v `docs/README.md`. Aktualni implementacni zaklad je `main` na verzi `4.3.0`; historicke `docs/PLAN-*` dokumenty pouzivej jen jako kontext. Pri produkcnim deployi vzdy zkontroluj, ze verze v `battle-plan/package.json`, viditelna verze v UI, Diagnostika a nasazeny bundle na `gh-pages` patri ke stejne zmene.
