...
9. Capability may report `drive_interop_probe.status=passed` only when it copies that receipt's authenticated message ID, canonical signed-body content digest and exact completion time. When following the link, first perform complete Ed25519 verification of capability and receipt with their respective trusted pairing records, including address, expiry and artifact checks; only then compare workspace, receipt ID/digest/time and contract artifact. Invalid signature returns the signature/trust error and can never reach link acceptance; an authenticated field mismatch is `drive_receipt_mismatch`.
