...
export function calendarEffectsForLocalTask(
    task: Pick<Task, 'googleEventId' | 'reservedGoogleEventId' | 'type'>,
    operation: 'upsert' | 'delete',
    allowUnlinkedUpsert = false,
): TaskEffectRequest[] {
    const isLinked = Boolean(task.googleEventId || task.reservedGoogleEventId);
...
export interface ExistingTaskReference {
    publicId?: string;
    localId?: number;
...
interface MutationPlan {
    ...
    current?: Task;
    task: Task;
    context: Required<Pick<TaskMutationContext, 'actor' | 'origin' | 'causeId'>> & { mutationId: string; googleAccountId?: string };
    revision: ProtocolRevision;
...
function isStale(task: Task | undefined, expectedRevision: string | null | undefined): boolean {
    if (expectedRevision === undefined) return false;
    if (expectedRevision === null) return currentRevision(task) !== null;
...
function effectRows(
    ...
    return requests.flatMap((request): AgentProtocolEffectRow[] => {
        const base = {
            ...
            accountId: task.googleAccountId,
            sequence: ++sequence,
        ...
        if (request.kind === 'calendar' && request.operation === 'upsert') {
            ...
            const payload: Extract<AgentProtocolEffectRow, { kind: 'calendar'; operation: 'upsert' }>['payload'] = {
                ...
                ...(task.totalDuration !== undefined ? { totalDuration: task.totalDuration } : {}),
                reservedEventId,
...
export class TaskMutationService {
    ...
    async updateTask(input: ExistingTaskReference & {
        ...
        return this.updateAs('update', input, input.changes);
    }
    ...
    async importTask(input: { task: Task; context: TaskMutationContext; localId?: number; expectedRevision?: string | null }): Promise<TaskMutationResult> {
        const request = structuredClone(input);
        return this.db.transaction('rw', taskMutationTables(this.db), async () => {
            const existing = request.localId != null
            ...
            if (existing) {
                ...
                // target for which this device already has durable delivery work.
                const hasTarget = Boolean(existing.googleEventId || existing.reservedGoogleEventId || existing.googleId);
                for (const field of ['googleEventId', 'reservedGoogleEventId', 'googleId', 'googleListId', 'googleAccountId'] as const) {
                    if (hasTarget || existing[field] !== undefined) Object.assign(task, { [field]: existing[field] });
                }
                for (const field of ['effectSequence', 'suggestionSubjectId', 'suggestionOccurrenceKey'] as const) {
    ...
    async queueEffects(input: ExistingTaskReference & {
        context: TaskMutationContext;
        effects: TaskEffectRequest[];
    }): Promise<{ status: 'queued'; effectIds: string[] } | { status: 'not_found' } | { status: 'stale'; currentRevision: string | null }> {
        const request = structuredClone(input);
        ...
        return this.db.transaction('rw', taskMutationTables(this.db), async () => {
            ...
            for (const effect of desired) {
                const latest = active.filter((candidate) => candidate.kind === effect.kind)
                    .sort((a, b) => b.sequence - a.sequence)[0];
                const existing = latest?.operation === effect.operation && latest.accountId === effect.accountId
                    && JSON.stringify(latest.payload) === JSON.stringify(effect.payload) ? latest : undefined;
                if (existing) effectIds.push(existing.id);
                else {
                    effect.sequence = (task.effectSequence ?? 0) + 1;
                    task.effectSequence = effect.sequence;
    ...
    private async findExisting(reference: ExistingTaskReference): Promise<Task | undefined> {
        if (reference.publicId) {
            const task = await this.db.tasks.where('publicId').equals(reference.publicId).first();
            return reference.localId != null && task?.id !== reference.localId ? undefined : task;
        }
    ...
    private async bindPendingEffects(task: Task): Promise<void> {
        if (!task.googleAccountId) return;
        await this.db.agentProtocolEffects.where('entityPublicId').equals(task.publicId!).modify((effect) => {
            if (!effect.accountId && ['pending', 'retry_scheduled'].includes(effect.state)) effect.accountId = task.googleAccountId;
        });
    }
    ...
    private async assertPlanStillCurrent(plan: MutationPlan): Promise<Task | undefined> {
        ...
        if (plan.current && (!current || current.publicId !== plan.current.publicId || currentRevision(current) !== currentRevision(plan.current))) {
...
export function newTaskMutationContext(
    ...
    return { actor, origin, causeId: crypto.randomUUID(), googleAccountId };
}
