...
export function ProjectManager({ onMessage }: ProjectManagerProps) {
    ...
    const handleCreate = async (confirmRestore = false) => {
        ...
        setStatus(null);
        try {
            const result = await createProject({ name, color, source: 'user', confirmRestore });
    ...
    const handleArchive = async (project: Project) => {
        ...
        const result = await archiveProject({ id: project.id, source: 'user' });
        setBusyId(null);
    ...
    const handleRestore = async (project: Project) => {
        if (!project.id) return;
        setBusyId(project.id);
        const result = await restoreProject({ id: project.id, source: 'user' });
        setBusyId(null);
    ...
    const handleColor = async (project: Project, nextColor: ProjectColor) => {
        ...
        setBusyId(project.id);
        const result = await updateProject({ id: project.id, color: nextColor, source: 'user' });
        setBusyId(null);
        ...
        <section aria-labelledby="project-manager-title" className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/30 p-4 sm:p-5">
            ...
            <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_10rem_auto]">
                <label className="min-w-0">
                    ...
                    <input
                        ...
                        className="w-full rounded-xl border border-slate-800 bg-slate-950/60 px-3 py-2.5 text-sm text-white outline-none placeholder:text-slate-600 focus:border-indigo-500"
                    />
                </label>
                <label>
            ...
            {pendingRestore && (
                <div className="mt-3 flex flex-col gap-2 rounded-xl border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-100 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <button type="button" onClick={() => void handleCreate(true)} className="shrink-0 rounded-lg bg-amber-500/20 px-3 py-2 font-black uppercase tracking-wider hover:bg-amber-500/30">Obnovit původní</button>
                </div>
            )}
